...
def connect_to_omero():
    ...
    if not hasattr(g,conf.OMERO_G_CONNECTION_KEY) or getattr(g,conf.OMERO_G_CONNECTION_KEY) is None:
        setattr(g,conf.OMERO_G_CONNECTION_KEY,OmeroConnection(host,port,token))

@conn_bp.after_request
def dissconnect_from_omero(response):

    if hasattr(g,conf.OMERO_G_CONNECTION_KEY) or getattr(g,conf.OMERO_G_CONNECTION_KEY) is not None:
